        ...
        for(action=0; action<ACTIONS_NUMBER; action++)
		{
			...
			if(child!=NULL){
				...
				switch(method){
					...
					// Insert this case in the switch-case within the First_InsertFrontier_Search_TREE function
                    case GeneralizedAStarSearch:
                        if (temp_node != NULL) {
                            // Update f(n) calculation with alpha factor
                            if (child->path_cost + alpha * child->state.h_n < temp_node->path_cost + alpha * temp_node->state.h_n) {
                                Remove_Node_From_Frontier(temp_node, &frontier);
                            } else {
                                continue;  // Child's cost is not better than the one in frontier
                            }
                        }
                        child->state.h_n = Compute_Heuristic_Function(&(child->state), goal_state);
...
void Insert_Priority_Queue_GENERALIZED_A_Star(Node *const child, Queue **frontier, float alpha) {
    Queue *temp_queue, *new_queue = (Queue*)malloc(sizeof(Queue));
    if (new_queue == NULL)
        Warning_Memory_Allocation(); 
    ...
    if (Empty(*frontier)) {
        *frontier = new_queue;  // Directly insert if empty
    } else {
        // Insert in sorted order based on f(n)
        if (child->path_cost + alpha * child->state.h_n < (*frontier)->node->path_cost + alpha * (*frontier)->node->state.h_n) {
            new_queue->next = *frontier;
            *frontier = new_queue;
        } else {
            for (temp_queue = *frontier; temp_queue->next != NULL; temp_queue = temp_queue->next) {
            ...
            if (temp_queue->next == NULL) {  // If child has the highest cost, add to the end
                temp_queue->next = new_queue;
            }
        }
    }
